...
def step6_group_data():
    ...
    # Convert timestamp - handle the specific format in our CSV
    def parse_timestamp(ts):
        try:
            # Handle "Wed Mar 04 2026 22:12:41 GMT+0530 (India Standard Time)" format
            return pd.to_datetime(ts)
        except:
            # Fallback to numeric if all else fails
    ...
    # Filter out zero loads for clean analysis
...
def step8_generate_graph(avg_tps):
    ...
    for i, (load, tps_value) in enumerate(avg_tps.items()):
        plt.annotate(f'{tps_value:.1f}', (load, tps_value), textcoords=(load, tps_value + 5), 
