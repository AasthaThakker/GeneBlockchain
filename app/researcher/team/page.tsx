...
export default function ResearcherTeamPage() {
    const { researcherId, institution, isAdmin } = useAuth()
    ...
    const fetchMembers = async () => {
        ...
        try {
            const res = await fetch(`/api/researcher-members?institution=${encodeURIComponent(institution)}`)
    ...
    useEffect(() => { fetchMembers() }, [institution])
    ...
    const handleAdd = async (e: React.FormEvent) => {
        ...
        setError(null)
        try {
            const res = await fetch("/api/researcher-members", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    institution,
                    adminResearcherId: researcherId,
            ...
            setSuccess("Researcher added successfully")
            clearForm()
            fetchMembers()
        } catch (err) {
    ...
    const handleEdit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const data = await res.json()
            if (!res.ok) throw new Error(data.error)
            setSuccess("Researcher updated successfully")
            clearForm()
            fetchMembers()
        } catch (err) {
            setError(err instanceof Error ? err.message : "Failed to update researcher")
        }
    }
    ...
    const handleDelete = async (userId: string, name: string) => {
        ...
        setError(null)
        try {
            const res = await fetch(`/api/researcher-members?userId=${userId}&adminResearcherId=${researcherId}`, { method: "DELETE" })
            const data = await res.json()
            if (!res.ok) throw new Error(data.error)
            setSuccess("Researcher removed")
            fetchMembers()
        } catch (err) {
            setError(err instanceof Error ? err.message : "Failed to remove researcher")
        }
    }

    const startEdit = (member: ResearcherMember) => {
        ...
        return (
            <DashboardShell role="researcher">
                <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <h2 className="text-xl font-semibold text-foreground">No University Found</h2>
                    <p className="mt-2 text-muted-foreground">
                        Your account is not associated with any research institution. <br />
        ...
        <DashboardShell role="researcher">
            <div className="mb-6 flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">{institution || "Your institution"}</p>
                </div>
                {isAdmin && (
                    <Button onClick={() => { clearForm(); setShowAddForm(true) }} className="gap-2">
                        <UserPlus className="h-4 w-4" /> Add Researcher
                    </Button>
                )}
            </div>
                ...
                <div className="mb-6 rounded-xl border border-border/50 bg-card p-6">
                    <div className="mb-4 flex items-center justify-between">
                        <h2 className="text-lg font-semibold text-foreground">
                            {editingId ? "Edit Researcher" : "Add New Researcher"}
                        </h2>
                    ...
                    <form onSubmit={editingId ? handleEdit : handleAdd} className="grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <Input id="name" value={formName} onChange={e => setFormName(e.target.value)} placeholder="e.g. Dr. Jane Doe" required={!editingId} />
                        </div>
                        <div>
                            <Label htmlFor="email">Email</Label>
                            <Input id="email" type="email" value={formEmail} onChange={e => setFormEmail(e.target.value)} placeholder="jane@university.edu" required={!editingId} />
                        </div>
                        {!editingId && (
                            <>
                                <div>
                        ...
                        <div className="sm:col-span-2">
                            <Button type="submit" className="gap-2">
                                ...
                                {editingId ? "Save Changes" : "Add Researcher"}
                            </Button>
                        </div>
                    </form>
                </div>
                ...
                <table className="w-full">
                    <thead>
                        <tr className="border-b border-border/50 bg-secondary/50">
                            ...
                            <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-muted-foreground">Researcher ID</th>
                    ...
                    <tbody className="divide-y divide-border/30">
                        {members.map(m => (
                            <tr key={m._id} className="transition-colors hover:bg-secondary/30">
                                ...
                                <td className="px-6 py-4 font-mono text-sm text-primary">{m.researcherId}</td>
                                <td className="px-6 py-4">
